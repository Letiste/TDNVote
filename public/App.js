...
const Admin = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<main class="${"svelte-1bon8hi"}"><div class="${"titleContainer svelte-1bon8hi"}"><h1 class="${"svelte-1bon8hi"}">Talents du Nord</h1></div>

  <div class="${"mainContainer svelte-1bon8hi"}"><h2 class="${"svelte-1bon8hi"}">Administration</h2>
    <div class="${"details svelte-1bon8hi"}"><button class="${"destroyVotes svelte-1bon8hi"}">Reset votes</button>
      <p class="${"svelte-1bon8hi"}">Nombre de votes : <strong>${escape(nbVotes)}</strong></p></div>

    <div>${each(cumulatedVotes, ({ ticketNumber, nbVote }) => `<p>${escape(ticketNumber)},
          <span class="${"votesBar svelte-1bon8hi"}" style="${"width:" + escape(nbVote * 300 / nbVotes) + "px; background-color:hsl(" + escape(nbVote * 360 / nbVotes) + ", 90%, 50%)"}"></span>${escape(nbVote)}
        </p>`)}</div>

    <div class="${"filterCategories svelte-1bon8hi"}"><label for="${"artists"}">Artistes</label>
      <button type="${"checkbox"}" style="${"margin-right: 30px; background-color: " + escape( "#ffde59" )}" class="${"svelte-1bon8hi"}"></button>

      <label for="${"spectators"}" style="${"margin-left: 30px"}">Spectateurs</label>
      <button style="${"background-color: " + escape( "#ffde59" )}" type="${"checkbox"}" class="${"svelte-1bon8hi"}"></button></div>

    <h3 class="${"svelte-1bon8hi"}">Votes</h3>

    <div class="${"tableWrapper svelte-1bon8hi"}"><table><tbody>${ `${each(artists, artist => `<tr class="${"svelte-1bon8hi"}"><td class="${"svelte-1bon8hi"}">N° de ticket: ${escape(artist.ticketNumber)}</td>
                <td class="${"svelte-1bon8hi"}">Vote: ${escape(artist.vote)}</td>
              </tr>`)}`
	}
          ${ `${each(spectators, spectator => `<tr class="${"svelte-1bon8hi"}"><td class="${"svelte-1bon8hi"}">N° de ticket: ${escape(spectator.ticketNumber)}</td>
                <td class="${"svelte-1bon8hi"}">Vote: ${escape(spectator.vote)}</td>
              </tr>`)}`
